...
+ (JSTileMap*)mapNamed:(NSString*)mapName withBaseZPosition:(CGFloat)baseZPosition andZOrderModifier:(CGFloat)zOrderModifier
{
	...
	// load the TMX map from disk - update
    NSString *path;
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    path = [[paths objectAtIndex:0] stringByAppendingPathComponent:mapName];
    //and update
	//org NSString* path = [[NSBundle mainBundle] pathForResource:name ofType:extension];
...
-(TMXLayer*)layerNamed:(NSString*)name
{
	for(TMXLayerInfo* layerInfo in self.layers)
	{
		if ([name isEqualToString:layerInfo.name])
            NSLog(@"layerinfo %@",layerInfo.name);
...
-(void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
	...
	else if([elementName isEqualToString:@"image"])
	{
		...
		else
		{
          ...
          //  [tileset setSourceImage:imageName];
}
	}
